...
async function callGasApi(action, payload = {}, password = "") {
    try {
        const response = await fetch(GAS_API_URL, {
            method: "POST",
            headers: { "Content-Type": "text/plain;charset=utf-8" }, // Evita problemas de CORS
...
function ActivityModal({ onClose }) {
    ...
    useEffect(() => {
        // REEMPLAZO DE GOOGLE.SCRIPT.RUN POR CALLAPI
...
function App() {
    ...
    const [data, setData] = useState({ canciones: [], equipo: [], servicios: [] });
    ...
    const handleLogin = () => {
        // Verificación local simple para UI, pero la API real valida al escribir datos
            ...
            ${view !== 'HOME' && html`
                <div className="sticky top-0 z-50 bg-[#020617]/95 backdrop-blur border-b border-white/5 p-4 flex items-center shadow-lg">
                    ...
                    <h2 className="font-bold text-white uppercase tracking-wider text-sm">
                        ${view === 'SONGS' ? 'Banco de Canciones' : view === 'TEAM' ? 'Equipo' : view === 'POSTER' ? 'Cronograma' : 'Servicio'}
                    </h2>
                </div>
            `}
                    ...
                    <div className="grid grid-cols-2 gap-3 mb-6">
                        ...
                        ${isAdmin ? html`
                            <button onClick=${() => {
                                setCurrentService({ id: null, fecha: new Date().toISOString().split('T')[0], jornada: 'Mañana', estado: 'Borrador', lider: '', coristas: [], musicos: [], repertorio: [] }); 
                                setView('SERVICE_EDITOR');
                            }} className="glass p-4 rounded-xl flex flex-col items-center gap-2 btn-active hover:bg-slate-800">
                                <${Icon.Plus} />
                                <span className="font-bold text-xs text-white">Crear Servicio</span>
                            </button>
                        ` : html`
                            <div className="glass p-4 rounded-xl flex flex-col items-center gap-2 opacity-50 cursor-not-allowed border border-slate-800">
                                <${Icon.Lock} />
                                <span className="font-bold text-xs text-slate-500">Crear (Solo Dir.)</span>
                            </div>
                        `}
                    </div>
...
function UpcomingServicesList({ servicios, isAdmin, onEdit, onNew }) {
        ...
        <div className="mb-8">
            ...
            <div className="space-y-3">
                ...
                ${upcoming.map(s => {
                    ...
                    return html`
                        <div key=${s.id} onClick=${() => onEdit(s)} className="glass p-4 rounded-xl flex justify-between items-center cursor-pointer border border-transparent hover:border-slate-500 transition relative overflow-hidden">
...
function ServiceEditor({ service, data, isAdmin, onSave, onDelete, onCancel }) {
    ...
    let filteredSongs = data.canciones.filter(c => c.titulo.toLowerCase().includes(searchSong.toLowerCase()));
    
    const groups = [
        { id: 'Rápida', icon: Icon.Fire, label: 'Alabanza / Rápidas', color: 'text-orange-400', border: 'border-orange-500' },
        { id: 'Lenta', icon: Icon.Dove, label: 'Adoración / Lentas', color: 'text-blue-400', border: 'border-blue-500' },
        { id: 'Ministración', icon: Icon.Hand, label: 'Ministración', color: 'text-purple-400', border: 'border-purple-500' },
        { id: 'Matrimonio', icon: Icon.Ring, label: 'Matrimonios', color: 'text-pink-400', border: 'border-pink-500' },
        { id: 'Eventos/Navidad', icon: Icon.Gift, label: 'Eventos / Navidad', color: 'text-green-400', border: 'border-green-500' }
    ];

    const isInSetlist = (songId) => form.repertorio.some(r => r.id === songId);

    const toggleSongInRepertory = (song) => {
        if (isInSetlist(song.id)) {
            const newRep = form.repertorio.filter(r => r.id !== song.id);
            ...
            ${tab === 'REPERTORIO' && html`
                <div className="space-y-4">
                            ...
                            ${form.repertorio.map((r, idx) => html`
                                <div key=${idx} className="bg-slate-800 p-2 rounded-lg flex justify-between items-center border-l-2 border-green-500">
                                    ...
                                    <div className="flex items-center gap-2 ml-2">
                                        ...
                                        <button onClick=${() => toggleSongInRepertory(r)} className="text-red-400 p-1"><${Icon.Trash}/></button>
                                    </div>
                                </div>
                            `)}
                        </div>
                    ...
                    <div className="border-t border-slate-700 pt-4">
                        <div className="sticky top-0 bg-[#020617] pb-2 z-10">
                            <input className="input-dark w-full" placeholder="🔍 Buscar canción..." value=${searchSong} onInput=${e => setSearchSong(e.target.value)} />
                        </div>
                        
                        <div className="space-y-4">
                            ${groups.map(group => {
                                const songsInGroup = filteredSongs.filter(s => s.ritmo === group.id);
                                ...
                                return html`
                                    <div>
                                        ...
                                        <div className="space-y-1">
                                            ${songsInGroup.map(c => {
                                                const selected = isInSetlist(c.id);
                                                return html`
                                                    <div onClick=${() => toggleSongInRepertory(c)} className=${`p-2 border rounded-lg cursor-pointer flex justify-between items-center transition ${selected ? 'selected-item border-yellow-500' : 'border-slate-800 hover:bg-slate-800'}`}>
                                                        <div className="flex items-center gap-3 overflow-hidden">
                                                            <div className=${`shrink-0 ${group.color}`}><${group.icon}/></div>
                                                            <div className="min-w-0">
                                                                <div className=${`text-sm font-bold truncate ${selected ? 'text-yellow-500' : 'text-slate-300'}`}><${SafeText} content=${c.titulo} /></div>
                                                                <div className="text-[10px] text-slate-500 truncate"><${SafeText} content=${formatTonoDisplay(c.tono)} /></div>
                                                            </div>
                                                        </div>
                                                        ${selected ? html`<div className="text-yellow-500"><${Icon.Check}/></div>` : html`<div className="text-slate-600 text-xs">+</div>`}
                                                    </div>
                                                `;
                                            })}
                                        </div>
                            ...
                            ${filteredSongs.filter(s => !groups.map(g=>g.id).includes(s.ritmo)).length > 0 && html`
                                <div class="pt-4 border-t border-slate-800">
                                    <h3 className="font-bold text-xs uppercase mb-2 flex items-center gap-1 text-slate-400"><${Icon.List}/> Otros / General (${filteredSongs.filter(s => !groups.map(g=>g.id).includes(s.ritmo)).length})</h3>
                                    <div className="space-y-1">
                                        ${filteredSongs.filter(s => !groups.map(g=>g.id).includes(s.ritmo)).map(c => { 
                                            const selected = isInSetlist(c.id); 
                                            return html`
                                                <div onClick=${() => toggleSongInRepertory(c)} className=${`p-2 border rounded-lg cursor-pointer flex justify-between items-center transition ${selected ? 'selected-item border-yellow-500' : 'border-slate-800 hover:bg-slate-800'}`}>
                                                    <div className="flex items-center gap-3 overflow-hidden">
                                                        <div className="text-slate-500 shrink-0"><${Icon.Music}/></div>
                                                        <div className="min-w-0">
                                                            <div className=${`text-sm font-bold truncate ${selected ? 'text-yellow-500' : 'text-slate-300'}`}><${SafeText} content=${c.titulo} /></div>
                                                            <div className="text-[10px] text-slate-500 truncate"><${SafeText} content=${formatTonoDisplay(c.tono)} /></div>
                                                        </div>
...
// --- SONG MANAGER ---
function SongManager({ data, equipo, isAdmin, refresh }) {
                        ...
                        <div>
                            ...
                            <div className="space-y-1">
                                ${songsInGroup.map(s => html`
                                    <div key=${s.id} onClick=${() => { if(isAdmin || true) { setSong({...s}); setMode('EDIT_SINGLE'); } }} className="glass p-3 rounded-xl flex items-center gap-3 mb-2 cursor-pointer border border-transparent hover:border-slate-500">
